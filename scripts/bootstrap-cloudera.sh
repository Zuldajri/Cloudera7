...
if [ "$INSTALLCDH" == "True" ]
then
  if ! sh initialize-cloudera-server.sh "$CLUSTERNAME" "$key" "$mip" "$worker_ip" "$HA" "$ADMINUSER" "$PASSWORD" "$CMUSER" "$CMPASSWORD" "$VMSIZE" "$CLUSTERNAME" "$NAMEPREFIX" "$LOCATION" >/dev/null 2>&1
  then
